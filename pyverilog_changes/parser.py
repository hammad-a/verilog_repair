...
class VerilogParser(PLYParser):
    ...
    def p_param(self, p):
        ...
        p[0] = Decl(list(paramlist), lineno=p.lineno(1))
        p.set_lineno(0, p.lineno(1))
    ...
    def p_decl(self, p):
        ...
        for rname, rdimensions in p[2]:
            decllist.extend(self.create_decl(p[1], rname, dimensions=rdimensions,
                                             lineno=p.lineno(2)))
        p[0] = Decl(list(decllist), lineno=p.lineno(1))
        p.set_lineno(0, p.lineno(1))
    ...
    def p_integerdecl(self, p):
        ...
        p[0] = Decl(list(intlist), lineno=p.lineno(1))
        p.set_lineno(0, p.lineno(1))
    ...
    def p_realdecl(self, p):
        ...
        p[0] = Decl(list(reallist), lineno=p.lineno(1))
        p.set_lineno(0, p.lineno(1))
    ...
    def p_instance_noname(self, p):
        ...
        p[0] = InstanceList(p[1], (), list(instancelist), lineno=p.lineno(1))
        p.set_lineno(0, p.lineno(1))
...
class NodeNumbering(ASTCodeGenerator):

    def __init__(self):
        self.c = -1
    
    def force_copy(self, ast):
        attr = vars(ast)
        for key in attr: # loop through all attributes of this AST
            if str(attr[key].__class__) in AST_CLASSES: # for each attribute that is also an AST
                attr[key] = copy.deepcopy(attr[key])
